...
X_searchspace = sobol_searchspace() #List of lists
print(type(X_searchspace))

obj=np.zeros(len(X_searchspace))
for i in range(len(X_searchspace)):
    obj[i] = objective_func(X_searchspace[i])

print("Maximum objective value found:", np.max(obj))